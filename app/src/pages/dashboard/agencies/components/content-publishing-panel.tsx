  ...
  const formBody = (
    <>
      <p className="text-xs text-muted">
        Source language: {getContentLanguageLabel(resolvedSource)}. Choose
        EstateWeb languages and how titles/descriptions are produced. OpenAI
        Batch delays CMS publish until the batch completes; otherwise titles
        generate in multi-property sync before enqueue.
      </p>
        ...
        <>
            ...
            <div className="flex flex-col gap-2 rounded-lg border border-border p-3">
              <div className="flex min-w-0 flex-col gap-0.5">
                ...
                <span className="text-xs text-muted">
                  Load another tracked agency&apos;s config into this form. Save
                  to keep.
                </span>
              </div>
                ...
                >
                  ...
                  <Select.Popover>
                    <ListBox items={otherTrackedAgencies}>
                      {(agency) => (
                        <ListBox.Item id={agency.id} textValue={agency.name}>
                          {agency.name}
                        </ListBox.Item>
                      )}
                    </ListBox>
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex min-w-0 flex-1 flex-col gap-0.5">
              ...
              <span className="text-xs text-muted">
                When off, CMS falls back to single-language original text.
              </span>
            </div>
            <Switch
            ...
            {outputs.map((output) => (
              ...
              >
                  ...
                  <Select.Popover>
                    <ListBox
                      items={TitleStrategyFormOptions.filter(
                        (option) =>
                          aiTitlesEnabled ||
                          option.id !== TitleProductionStrategies.AI,
                      )}
                    >
                      {(option) => (
                        <ListBox.Item
                ...
                >
                  ...
                  <Select.Popover>
                    <ListBox items={[...DescriptionStrategyFormOptions]}>
                      {(option) => (
                        <ListBox.Item
                          id={option.id}
                          textValue={option.label}
        ...
        >
          <Modal.Container>
            <Modal.Dialog className="max-h-[90vh] w-full max-w-3xl">
              ...
              <Modal.Footer>{formActions}</Modal.Footer>
            </Modal.Dialog>
          </Modal.Container>
        </Modal.Backdrop>
      </Modal>
