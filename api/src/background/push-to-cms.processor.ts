...
import { PUSH_TO_CMS_QUEUE } from '@/core/queues/queues.constants';
import { JobStatus } from 'generated/prisma';
import { PushToCmsJobService } from '@/modules/user-properties/services/push-to-cms-job.service';
import {
  PushToCmsItemResult,
  PushToCmsJobData,
  PushToCmsJobResult,
} from '@/modules/user-properties/interfaces/push-to-cms-job.interface';

const PUSH_TO_CMS_WORKER_CONCURRENCY = 5;

@Processor(PUSH_TO_CMS_QUEUE, { concurrency: PUSH_TO_CMS_WORKER_CONCURRENCY })
export class PushToCmsProcessor extends WorkerHost implements OnModuleInit {
  private readonly logger = new Logger(PushToCmsProcessor.name);

  constructor(
    private readonly prisma: PrismaService,
    private readonly pushToCmsJobService: PushToCmsJobService,
  ) {
    super();
  }

  async onModuleInit(): Promise<void> {
    this.worker.concurrency = PUSH_TO_CMS_WORKER_CONCURRENCY;
  }

  async process(job: Job<PushToCmsJobData>): Promise<void> {
    ...
    try {
      const item = await this.pushToCmsJobService.processProperty(job.data);
  ...
  private emptyResult(total: number): PushToCmsJobResult {
    return {
      total,
      processed: 0,
      queued: 0,
  ...
  private async ensureJobActive(
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      const result =
        (log.result as unknown as PushToCmsJobResult | null) ??
  ...
  private async recordItemResult(
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      result.queued = result.items.filter(
      ...
      const hardFailed =
        finished && result.queued === 0 && result.failed === result.total;
          ...
          ...(finished
            ? {
                ...
                error_message: hardFailed
                  ? `CMS push failed for all ${result.total} properties`
                  : result.failed > 0
