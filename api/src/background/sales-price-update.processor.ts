...
import { SALES_PRICE_UPDATE_QUEUE } from '@/core/queues/queues.constants';
import { JobStatus } from 'generated/prisma';
import { SalesPriceUpdateJobService } from '@/modules/user-properties/services/sales-price-update-job.service';
import {
  SalesPriceUpdateItemResult,
  SalesPriceUpdateJobData,
  SalesPriceUpdateJobResult,
} from '@/modules/user-properties/interfaces/sales-price-update-job.interface';

const SALES_PRICE_UPDATE_WORKER_CONCURRENCY = 5;

@Processor(SALES_PRICE_UPDATE_QUEUE, {
  concurrency: SALES_PRICE_UPDATE_WORKER_CONCURRENCY,
})
export class SalesPriceUpdateProcessor
  extends WorkerHost
  implements OnModuleInit
{
  private readonly logger = new Logger(SalesPriceUpdateProcessor.name);

  constructor(
    private readonly prisma: PrismaService,
    private readonly salesPriceUpdateJobService: SalesPriceUpdateJobService,
  ) {
    super();
  }

  async onModuleInit(): Promise<void> {
    this.worker.concurrency = SALES_PRICE_UPDATE_WORKER_CONCURRENCY;
  }

  async process(job: Job<SalesPriceUpdateJobData>): Promise<void> {
    ...
    try {
      const item =
        await this.salesPriceUpdateJobService.processProperty(job.data);
  ...
  private emptyResult(total: number): SalesPriceUpdateJobResult {
    return {
      total,
      processed: 0,
      updated: 0,
  ...
  private async ensureJobActive(
    logId: string,
    job: Job<SalesPriceUpdateJobData>,
    total: number,
  ): Promise<void> {
    await this.prisma.$transaction(async (tx) => {
      ...
      const result =
        (log.result as unknown as SalesPriceUpdateJobResult | null) ??
  ...
  private async recordItemResult(
    logId: string,
    item: SalesPriceUpdateItemResult,
    total: number,
  ): Promise<void> {
    await this.prisma.$transaction(async (tx) => {
      await tx.$executeRaw(
          ...
          ...(finished
            ? {
                ...
                error_message: hardFailed
                  ? `Sales price update failed for all ${result.total} properties`
                  : result.failed > 0
