...
import { CREATE_INTEGRATION_IMAGES_QUEUE } from '@/core/queues/queues.constants';
import { JobStatus } from 'generated/prisma';
import { CreateIntegrationImagesJobService } from '@/modules/user-properties/services/create-integration-images-job.service';
import {
  CreateIntegrationImagesItemResult,
  CreateIntegrationImagesJobData,
  CreateIntegrationImagesJobResult,
} from '@/modules/user-properties/interfaces/create-integration-images-job.interface';

const CREATE_INTEGRATION_IMAGES_WORKER_CONCURRENCY = 5;

@Processor(CREATE_INTEGRATION_IMAGES_QUEUE, {
  concurrency: CREATE_INTEGRATION_IMAGES_WORKER_CONCURRENCY,
})
export class CreateIntegrationImagesProcessor
  extends WorkerHost
  implements OnModuleInit
{
  private readonly logger = new Logger(CreateIntegrationImagesProcessor.name);

  constructor(
    private readonly prisma: PrismaService,
    private readonly createIntegrationImagesJobService: CreateIntegrationImagesJobService,
  ) {
    super();
  }

  async onModuleInit(): Promise<void> {
    this.worker.concurrency = CREATE_INTEGRATION_IMAGES_WORKER_CONCURRENCY;
  }

  async process(job: Job<CreateIntegrationImagesJobData>): Promise<void> {
    ...
    try {
      const item =
        await this.createIntegrationImagesJobService.processProperty(job.data);
  ...
  private emptyResult(total: number): CreateIntegrationImagesJobResult {
    return {
      total,
      processed: 0,
      created: 0,
  ...
  private async ensureJobActive(
    logId: string,
    job: Job<CreateIntegrationImagesJobData>,
    total: number,
  ): Promise<void> {
    await this.prisma.$transaction(async (tx) => {
      ...
      const result =
        (log.result as unknown as CreateIntegrationImagesJobResult | null) ??
  ...
  private async recordItemResult(
    logId: string,
    item: CreateIntegrationImagesItemResult,
    total: number,
  ): Promise<void> {
    await this.prisma.$transaction(async (tx) => {
      await tx.$executeRaw(
      ...
      result.created = result.items.filter(
          ...
          ...(finished
            ? {
                ...
                error_message: hardFailed
                  ? `Image push to CMS failed for all ${result.total} properties`
                  : result.failed > 0
