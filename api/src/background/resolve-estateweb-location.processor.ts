    ...
    await this.prisma.$executeRaw`
      UPDATE job_logs j SET
        ...
        status = CASE
          ...
          WHEN counts.resolved = 0 AND counts.failed = ${total}::int THEN 'FAILED'::"JobStatus"
        ...
        error_message = CASE
          ...
          WHEN counts.resolved = 0 AND counts.failed = ${total}::int
      ...
      FROM (
        SELECT
          ...
          COUNT(*) FILTER (WHERE status IN ('resolved', 'unchanged', 'skipped'))::int AS resolved,
