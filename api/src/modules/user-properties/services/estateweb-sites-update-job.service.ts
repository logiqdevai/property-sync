...
@Injectable()
export class EstateWebSitesUpdateJobService {
  private readonly logger = new Logger(EstateWebSitesUpdateJobService.name);
  ...
  async processProperty(
    data: EstateWebSitesUpdateJobData,
  ): Promise<EstateWebSitesUpdateItemResult> {
    ...
    try {
      ...
      this.logger.log(
        `[processProperty] pushUpdate sites property=${data.user_property_id} integration=${userIntegrationId} sites=${data.sites.length}`,
      );
