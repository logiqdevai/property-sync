...
2. `dto/create-agency.dto.ts` — `class-validator`: `name` (string, required),
   ...
   strings), `crawl_interval` (optional string, defaults to
   `"0 */6 * * *"` in the service if omitted — validate it's a plausible cron
   string), `notes` (optional string)
...
4. `dto/agency-query.schema.ts` — Zod schema: `page` (default 1), `limit`
   ...
   `country`/`city` (optional exact match)
...
6. `agencies.controller.ts` — `@Controller('admin/agencies')`, all routes
   ...
   except `create`/`update`/`updateStatus`/`remove` which are
   `@Roles('ADMIN', 'SUPER_ADMIN')` only (`SUPPORT` is read-only per
...
- [ ] Implement all 6 endpoints from `directions/04-api-design.md`
